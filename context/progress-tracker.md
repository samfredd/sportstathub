...
| 42 | **Stat tiering — free vs Pro pills on match detail** (from competitor-analysis video review). Free tier keeps the commodity stats every livescore site carries: Goals, Corners, Shots, Shots on Target. Pro tier (gold crown pill, click → UpgradeModal, gated via `advanced_stats` flag with PremiumGate-identical access rules): Cards, Red Cards, Fouls, Offsides, Possession, xG, Shots off Target, Blocked Shots, Pass Accuracy, Saves. Added 4 new provider-backed stat types (Shots off Goal / Blocked Shots / Red Cards / Passes %); throw-ins/interceptions/dribbles/crosses deliberately excluded — API-Football never returns them. Percentage stats generalized via `PCT_STATS` (possession + pass accuracy: % suffix, no meaningless total). Verified in preview: locked pill opens modal without switching stat, free pills work, no console errors | 2026-07-02 |
| 48 | **SECURITY (critical) — auth bypass in `/auth/verify-otp` closed.** The "already verified" branch issued a JWT + session cookie **without checking the OTP**, so anyone who knew a registered email could obtain a valid session for that account. Now returns 409 "Account is already verified — please sign in"; verified users authenticate through `/auth/login`. Verified live: 409 + no token. | 2026-07-02 |
| 47 | **SECURITY — `optionalAuth` now reads the httpOnly cookie.** It only inspected the `Authorization` header, but sessions moved to cookies — so signed-in Pro users hit `/api/predictions`, `/api/predictions/:id`, `/api/comments` as anonymous and had premium content masked. Now verifies the cookie (or header), attaches the user when valid, and never rejects (public routes still serve anonymous). | 2026-07-02 |
| 46 | **Likes — per-user dedupe + toggle (was unlimited + unauthenticated).** `/api/{predictions,forum/threads,comments}/:id/like` had no auth and bumped a bare counter, so one visitor could inflate a count 200×/min. Added `014_content_likes.sql` (unique `(user_id, content_type, content_id)`), rewrote the repo to toggle like/unlike in a transaction (counter clamped ≥0), required auth on all three routes, frontend redirects to login on 401. Verified: toggle `liked:true→false`, count 19→18, no double-count. | 2026-07-02 |
| 45 | **Payments — history endpoint + verify race hardened.** Added `GET /api/billing/history` (+ `findPaymentsByUser`) and a Payment History table on the subscription page. Closed the webhook-vs-user verify race with `claimPaymentForProcessing` (atomic `UPDATE … RETURNING`, reclaimable after 2 min); the loser reads settled state or gets 409, frontend retries once on 409. Added 2 service tests (race-loss 409, settled-readback). | 2026-07-02 |
| 44 | **Caching — cross-user leak closed + dead code removed.** `communityApi`'s sessionStorage cache scoped only "auth" vs "anon", so user A's cached responses could serve user B on a shared device. `session.ts` now purges the `sportstathub:api:` cache on every login/logout/identity change. Removed the dead `getAuthHeaders()` no-op path from `api.ts`. Backend Redis TTLs + public/private `Cache-Control` scoping reviewed — gated routes correctly use `private`. | 2026-07-02 |
| 43 | **Audit — confirmed free/premium stat tiering matches product intent** (see #42). Commodity stats free, analytical stats Pro, fail-closed on missing flag, admin override, no lock-flash. No code change needed. | 2026-07-02 |
